...
- **The series shelf** ([/series](https://kernels.rudrite.com/series)): multi-page runs that take one subject past chapter depth. Three so far: the machine (TPU and GPU silicon, the generations, both fabrics, with three instruments built for it), Pallas the language, and XLA from the inside, down to the symbol a PJRT plugin exports.
