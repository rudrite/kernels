...
- **Lessons**: 52 lessons nested inside the chapters that own them, each ending in a check and carrying mastery work that rolls up into the chapter's ledger: the machine, the profiler, the fabric, and the Pallas language in the kernel path; the compiler internals in the XLA path, down to the symbol a PJRT plugin exports.
