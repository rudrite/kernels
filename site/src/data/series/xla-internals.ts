// The xla-internals series: seven pages past the XLA path, at interface and
// implementation level. C snippets are quoted from the public tree at the
// URLs in the working notes; HLO excerpts are verbatim from this repo's own
// captures (hlo-pairs.json, TPU v6 lite, jax 0.11.0; the CPU and TPU pass
// corpora under data/xla/). The bar: a reader could start their own PJRT
// plugin and read xla source with a map.
import type { Series } from './index'

export const XLA_INTERNALS_SERIES: Series = {
  id: 'xla-internals',
  title: 'XLA from the inside',
  lede: 'The path walks the compiler at chapter level. This series reads the interfaces: the one exported symbol, the invariants, the passes as filenames, and the seam where the TPU backend leaves the public tree.',
  deepens: [
    { label: 'the xla path, all fourteen chapters', href: '/xla' },
    { label: 'ch 01 · pjrt', href: '/xla/pjrt' },
    { label: 'ch 04 · the pass pipeline', href: '/xla/pipeline' },
  ],
  pages: [
    // ------------------------------------------------------------------ 1
    {
      ...
      sections: [
        {
          ...
          ps: [
            "The path opens at this seam and stops there, which is the right altitude for a chapter whose job is orientation: /xla/pjrt names the boundary, names the three nouns on either side of it, and moves on. This page starts one line below that, in the symbol table of a plugin's shared object. Read the linker version script XLA ships for its own CPU plugin and the entire public surface of a PJRT backend turns out to be a single name. GetPjrtApi is global. Everything else in the library is local.",
        ...
        },
        {
          ...
          ps: [
            ...
            "So the difference between a toy plugin and libtpu is not the interface. It is what sits behind the function pointers, which is a compiler and a runtime for a chip nobody outside Google can read. Page 6 of this series takes that seam apart properly.",
          ],
        },
      ],
      readings: [
        { label: 'the PJRT C API header', url: 'https://github.com/openxla/xla/blob/main/xla/pjrt/c/pjrt_c_api.h', note: 'every struct this page quotes, in its own words' },
      ...
      sections: [
        {
          ...
          ps: [
            "The path separates these two at /xla/ingestion and then reads HLO at /xla/hlo, and the split it teaches is correct: StableHLO is the wire format, HLO is the workbench. What that pairing leaves out is how differently the two are specified, which turns out to matter every time you go looking for the rules.",
      ...
      sections: [
        {
          ...
          ps: [
            ...
            "That bool is the reason a dump has repeats in it. A pipeline can be wrapped so that it reruns its member passes until every one of them returns false, and the path notices this at /xla/pipeline when algsimp shows up several times in a row. The mechanism behind the observation is a loop reading return values, and there is a companion entry point for it, `RunOnChangedComputations`, which lets a pass in a fixpoint loop revisit only what the previous iteration touched instead of the whole module.",
      ...
      sections: [
        {
          ...
          ps: [
            "/xla/fusion makes the load-bearing argument, that fusion changes where intermediates live and never changes the math, and it is the right thing to hold onto. What a chapter does not have room for is the vocabulary, and the vocabulary is small enough to learn in one sitting. XLA distinguishes four kinds of fusion, and it prints the kind on every fusion instruction it emits.",
    ...
    // ------------------------------------------------------------------ 6
    {
      ...
      num: 6,
      work: [
        { id: 'seam', label: 'list xla/backends yourself and place libtpu in the picture page 1 drew' },
      ],
      ...
      sections: [
        ...
        },
        {
          ...
          ps: [
            ...
            "The fourth thing in there is the one page 1 established. It is a PJRT plugin, exporting GetPjrtApi, discovered by JAX through the same mechanism as any other plugin. Every layer above it, IFRT, jax.jit, your model code, is talking to a function-pointer struct and does not know or care that a compiler is on the other side.",
        ...
        },
        {
          ...
          ps: [
            "Now the question this page exists for. If you are building a backend, what do you have to write? The contract is narrower than the size of the XLA tree suggests, and the reason is in a struct from page 1. PJRT_Client_Compile takes a PJRT_Program, and a PJRT_Program is a code blob plus a `format` string. Nothing in that contract says the code is HLO. Nothing says XLA's pass pipeline runs.",
            ...
            "There is a conformance harness for the interface itself. `RegisterPjRtCApiTestFactory` gives a plugin the standard tests of basic PJRT C API behaviours, and the integration guide's own acceptance criteria are ordinary JAX operations: element-wise arithmetic, a jit compile, and a pmap with a collective reduction. That last one is the real gate, because it exercises the multi-device path that the key-value callbacks on page 1 exist to bootstrap.",
          ],
          table: {
        ...
        },
        {
          ...
          ps: [
            ...
            "Either way the surface a framework sees is identical, and that is the property page 7 depends on. A layer above PJRT can treat every backend as the same kind of object precisely because the seam was drawn at a struct of function pointers rather than at an IR.",
          ],
        },
      ],
      readings: [
    ...
    // ------------------------------------------------------------------ 7
    {
      ...
      sections: [
        ...
        },
        {
          ...
          ps: [
            "This series went down the stack and back up it: the exported symbol on page 1, the invariants a pass may not break on page 2, the pipeline as filenames on page 3, the fusion queue on page 4, the partitioner on page 5, the seam where backends part ways on page 6, and the layer above all of it here. What is left is not more series pages. It is the path's own closing chapters and the work in them.",
      ...
      readings: [
        ...
        { label: 'PJRT on openxla.org', url: 'https://openxla.org/xla/pjrt', note: 'the layer underneath, where the series started' },
      ],
    },
  ],
}
