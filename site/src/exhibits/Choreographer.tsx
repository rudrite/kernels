  ...
  return (
    <div className="choreo">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${trace.title}, frame ${i + 1} of ${trace.frames.length}: ${frame.caption}`}>
        <text x={8} y={10} fill={TEXT_MUTE} fontSize={10.5} letterSpacing={1} fontFamily="inherit">
          HBM
        </text>
        {/* HBM groups */}
        {groups.map((g, gi) => {
          ...
          return (
            <g key={g}>
              <text x={8} y={16 + gi * groupH + 12} fill={remote ? STEEL : TEXT_MUTE} fontSize={10.5} letterSpacing={1} fontFamily="inherit">
                {remote ? `${g} · ICI` : g}
              </text>
              {trace.hbmTiles
                .filter((t) => t.group === g)
                .map((t) => {
                  ...
                  return (
                    <g key={t.id} opacity={dead ? 0.45 : 1}>
                      <rect x={p.x} y={p.y} width={TILE} height={TILE} fill={hot ? '#1a1d21' : 'transparent'} stroke={hot ? STEEL : IDLE_EDGE} strokeWidth={hot ? 1.5 : 1} strokeDasharray={remote ? '3 2' : undefined} />
                      <text x={p.x + TILE / 2} y={p.y + TILE / 2 + 3} textAnchor="middle" fill={hot ? PANEL_INK : TEXT_MUTE} fontSize={11.5} fontFamily="inherit">
                        {t.id}
                      </text>
        ...
        {/* VMEM slots */}
        <text x={slotX} y={10} fill={TEXT_MUTE} fontSize={10.5} letterSpacing={1} fontFamily="inherit">
          VMEM
        </text>
        {trace.slotIds.map((s) => {
          const p = slotPos(s)
          ...
          return (
            <g key={s}>
              <rect x={p.x} y={p.y} width={isAcc ? 190 : 150} height={TILE} fill={occupant ? '#1a1d21' : 'transparent'} stroke={occupant ? (isAcc ? COPPER : PANEL_INK) : IDLE_EDGE} strokeWidth={1} />
              <text x={p.x + 8} y={p.y + TILE / 2 + 3} fill={TEXT_MUTE} fontSize={10} fontFamily="inherit">
                {s}
              </text>
              <text x={p.x + 42} y={p.y + TILE / 2 + 3} fill={occupant ? (isAcc ? COPPER : PANEL_INK) : TEXT_MUTE} fontSize={11} fontFamily="inherit">
                {occupant ?? 'empty'}
              </text>
            </g>
          )
        ...
        {trace.stateIds.length > 0 && (
          <g>
            <text x={slotX} y={16 + trace.slotIds.length * (TILE + GAP) + 8} fill={TEXT_MUTE} fontSize={10.5} letterSpacing={1} fontFamily="inherit">
              CARRIED STATE
            </text>
            {trace.stateIds.map((sid, si) => (
              <text key={sid} x={slotX} y={16 + trace.slotIds.length * (TILE + GAP) + 28 + si * 20} fill={COPPER} fontSize={11.5} fontFamily="inherit">
                {sid} : {frame.state?.[sid] ?? '·'}
              </text>
            ))}
          </g>
        ...
        {(() => {
          ...
          return (
            <g>
              ...
              <text x={slotX + 8} y={y + 19} fill={active ? COPPER : TEXT_MUTE} fontSize={11.5} fontFamily="inherit">
      ...
      <style>{`
        ...
        .choreo .caption { font-size: 0.8125rem; color: ${PANEL_INK}; padding: 0.625rem 0.875rem 0; min-height: 3.2em; margin: 0; }
